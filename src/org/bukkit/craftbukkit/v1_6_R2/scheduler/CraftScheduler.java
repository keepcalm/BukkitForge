package org.bukkit.craftbukkit.v1_6_R2.scheduler;
...
public class CraftScheduler implements org.bukkit.scheduler.BukkitScheduler {
	...
	public CraftTask runTask(Plugin plugin, Runnable runnable) {
	...
	public CraftTask runTaskAsynchronously(Plugin plugin, Runnable runnable) {
	...
	public CraftTask runTaskLater(Plugin plugin, Runnable runnable, long delay) {
	...
	public CraftTask runTaskLaterAsynchronously(Plugin plugin, Runnable runnable, long delay) {
	...
	public CraftTask runTaskTimer(Plugin plugin, Runnable runnable, long delay, long period) {
	...
	public CraftTask runTaskTimerAsynchronously(Plugin plugin, Runnable runnable, long delay, long period) {
		validate(plugin, runnable);
		if (delay < 0l) {
			delay = 0;
		}
	...
	public void mainThreadHeartbeat(final int currentTick) {
		CraftScheduler.currentTick = currentTick;
		...
		while (isReady(currentTick)) {
			...
			if (task.isSync()) {
				try {
					...
					if (task.getOwner() instanceof CraftDummyPlugin) {
						System.out.println("WARNING: Internal task failed: ");
