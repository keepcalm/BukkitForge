...
class BukkitMetaPotion extends BukkitMetaItem implements PotionMeta {
    ...
    BukkitMetaPotion(BukkitMetaItem meta) {
        super(meta);
        if (!(meta instanceof BukkitMetaPotion)) {
            return;
        }
        BukkitMetaPotion potionMeta = (BukkitMetaPotion) meta;
        if (potionMeta.hasCustomEffects()) {
            this.customEffects = new ArrayList<PotionEffect>(potionMeta.customEffects);
        }
    }

    BukkitMetaPotion(NBTTagCompound tag) {
        super(tag);
    ...
    BukkitMetaPotion(Map<String, Object> map) {
        super(map);
    ...
    @Override
    public BukkitMetaPotion clone() {
        BukkitMetaPotion clone = (BukkitMetaPotion) super.clone();
        if (hasCustomEffects()) {
    ...
    int applyHash() {
        ...
        return original != hash ? BukkitMetaPotion.class.hashCode() ^ hash : hash;
    }
    ...
    public boolean equalsCommon(BukkitMetaItem meta) {
        ...
        if (meta instanceof BukkitMetaPotion) {
            BukkitMetaPotion that = (BukkitMetaPotion) meta;
    ...
    boolean notUncommon(BukkitMetaItem meta) {
        return super.notUncommon(meta) && (meta instanceof BukkitMetaPotion || isPotionEmpty());
    }
